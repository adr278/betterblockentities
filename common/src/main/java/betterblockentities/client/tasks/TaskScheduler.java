...
public class TaskScheduler {
    ...
    public static void schedule(@NonNull Runnable task) {
        Minecraft.getInstance().execute(() -> {
            try {
                ...
                GlobalScope.LOGGER.error("Post-reload task failed", t);
            }
        });
    }
    ...
    public static void scheduleOnReload(ReloadInstance reload, @NonNull Runnable task) {
        if (reload == null) {
            GlobalScope.LOGGER.error("Resource reload instance was null! {}", reload);
            return;
        }
        ...
        reload.done().whenComplete((ignored, err) -> {
            ...
            if (err != null) {
                GlobalScope.LOGGER.error("Resource reload future completed exceptionally", err);
                return;
            }
            ...
            try {
                ...
                GlobalScope.LOGGER.error("Resource reload finished with errors", t);
                return;
            }
