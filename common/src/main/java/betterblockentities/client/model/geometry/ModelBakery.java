...
public class ModelBakery {
    ...
    public static void bakeLayerSetupAndAppend(EntityModelSet entityModelSet, ModelLayerLocation layer, PoseStack stack) {
        ...
        if (root.getAllParts().isEmpty()) {
            GlobalScope.LOGGER.error("Root ModelPart for ModelLayer {} is empty after bake! Skipping", layer.layer());
            return;
        }
        ...
        else if (layer == SupportedModelLayers.SIGN_STANDING ||
                layer == SupportedModelLayers.SIGN_WALL) {
            setupSigns(layer, root, stack);
        }

        else if (layer == SupportedModelLayers.HANGING_SIGN_WALL    ||
                 layer == SupportedModelLayers.HANGING_SIGN_CEILING ||
                 layer == SupportedModelLayers.HANGING_SIGN_CEILING_MIDDLE) {
    ...
    private static void setupBed(ModelLayerLocation layer, ModelPart root, PoseStack stack) {
        GeometryRegistry.cacheGeometry(layer, root,
                layer == ModelLayers.BED_HEAD ?
                        PlaceHolderSpriteIdentifiers.BED_HEAD :
                        PlaceHolderSpriteIdentifiers.BED_FOOT,
                stack);
    }
    ...
    private static void setupSigns(ModelLayerLocation layer, ModelPart root, PoseStack stack) {
        if (layer == SupportedModelLayers.SIGN_STANDING) {
            GeometryRegistry.cacheGeometry(layer, root, PlaceHolderSpriteIdentifiers.SIGN, stack);
        } else if (layer == SupportedModelLayers.SIGN_WALL) {
            GeometryRegistry.cacheGeometry(layer, root, PlaceHolderSpriteIdentifiers.SIGN, stack);
        }
    }

    private static void setupHangingSigns(ModelLayerLocation layer, ModelPart root, PoseStack stack) {
        GeometryRegistry.cacheGeometry(layer, root, PlaceHolderSpriteIdentifiers.HANGING_SIGN, stack);
    }
